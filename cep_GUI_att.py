...
def consulta_cep():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "erro" in dados:
                messagebox.showerror("Aviso!", "CEP não encontrado")

            else:
                endereco = (
                            f"Logradouro: {dados.get('logradouro', 'N/A')} \n"
                            f"Bairro: {dados.get('bairro', 'N/A')} \n"
                            f"Localidade: {dados.get('localidade', 'N/A')} \n"
            ...
            messagebox.showerror("Aviso!", "Erro ao consultar serviços!")
...
form.iconbitmap("maps.ico")
form.configure(background="white")

#Imagem
imagem = Image.open("ponto.ico")
...
#Entrada
entrada_cep = tk.Entry(form, width=20, font=("Arial", 10))
